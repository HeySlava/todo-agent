...
Для того, чтобы классифицировать задачу как "todo", а запросе обязательно должна быть указана задача: что нужно сделать, и дата, когда нужно это сделать. Если даты нет, но указан интервал времени, тебе необходимо определить определить date самостоятельно путем сложения текущей даты и интервала. Если чего-то из этого не хватает, ты не можешь ответить полностью на поставленный вопрос. Для "payload" должен быть словарь, где "summary" — это задача, которую я прошу выполнить, а "date" — значение в формате день:месяц.год часы:минуты.  # noqa: E501
